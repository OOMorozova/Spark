...
# [Spark. Потоковая обработка](https://stepik.org/course/183213/syllabus)

[2.3.1 Structured Streaming](src/main/scala/streaming/Iot.scala)
> Необходимо каждую минуту на протяжении двух минут отслеживать среднюю температуру (temp) окружающей среды (в градусах Цельсия), фиксируемую каждым устройством (device).
> Должна быть организована потоковая обработка данных.
> Результат должен выводиться на экран.
> В имеющихся данных температура указана в градусах Фаренгейта. 

[2.3.2 Structured Streaming](src/main/scala/streaming/News.scala)
> Необходимо организовать потоковую обработку данных для получения следующей информации:
> количества новостей, каждый день размещаемых каждым сайтом в каждой категории.
> Учитывайте, что  вместо кода категории требуется показывать полное название категории (category_name)
> Итоговую информацию следует выводить на экран
 
[2.3.3. Работа с состоянием](src/main/scala/streaming/Rate.scala)
> для каждых десяти элементов value требуется выводить их среднее значение
 
[2.3.4. Работа с состоянием](src/main/scala/streaming/Taxi.scala)
> для каждого клиента требуется выводить информацию о полной стоимости каждой его поездки на такси (fare + tips)
> при этом учитывайте, что каждая третья поездка в месяц идет со скидкой в 10%, 
> но только если все три раза клиент расплачивается картой (в payment_type указано CreditCard)