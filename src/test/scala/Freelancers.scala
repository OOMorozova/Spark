...
import org.apache.spark.sql.functions.{concat, lit, col, abs, avg, rand, explode}
...
//  val freelancersDF2 = freelancersDF
//    .crossJoin(saltDF)

//    .withColumn("salt", lit(rand() * num_of_partitions % num_of_partitions) )
//
//  val offersDF2 = offersDF
//  .withColumn("salt", (lit(skewFactor) * rand()).cast("int"))
//    .withColumn("salt",  explode(lit(range(1, num_of_partitions))))
